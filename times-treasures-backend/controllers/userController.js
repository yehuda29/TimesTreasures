...
/**
 * @desc    Send a contact message from user's email to the company's inbox,
 *          and send a reply to the user confirming receipt.
...
exports.sendContactMessage = asyncHandler(async (req, res) => {
  ...
  try {
    // Build the email that goes to the company's inbox
    const mailOptions = {
      from: `${name} <${email}>`,         // Appear to come from the user
    ...
    // Build the reply email to the user
    const replyMailOptions = {
      from: process.env.EMAIL_USER, // Your site email
